...
The default suite is self-contained. Ordinary staging checks are opt-in and read-only. The separately gated signup write project exists but its local ledger is disabled after ten consumed attempts. An approved lifecycle with a numeric cap, unique data, owner, ledger, and retention or cleanup policy is necessary but no longer sufficient for execution: no agent may submit another signup, create accounts, mutate APIs, or add staging data without a new explicit human decision covering that action. Production execution remains disabled. Keep real personal data, credentials, tokens, and sensitive artifacts out of commits and external services. A planned production case or UAT label is not execution authorization.
