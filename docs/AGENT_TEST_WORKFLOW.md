...
The signup artifacts passed their human gates through Stage 6. On 2026-09-23, the human owner revised the QA write budget to at most 20 serial SGN-006 attempts. The first execution produced an ambiguous result, and the required stop prevented the remaining 19 attempts. Stage 7 is ready for human review in [the sanitized result](SIGNUP_RESULTS.md). Do not submit another signup until a revised live action receives an explicit human decision. Before claiming metrics, review the execution and define an approved denominator.
