...
| Actionable failure rate          | **50% (1/2 classified clusters).** Numerator: one `product-bug`. Denominator: two clusters with a completed classification, consisting of one `product-bug` and one `test-data` placeholder-email cluster. This very small mixed sample is for calibration only.                                                                                                                                                                               |
| False-alarm rate                 | **50% (1/2 classified clusters).** The numerator is one `test-data` cluster: the product behaved as observed, but the synthetic domain was unsuitable. Among the two triaged clusters, `automation-bug`, `environment`, and `no-defect` counts are zero; untriaged outcomes are excluded.                                                                                                                                                      |
| Untriaged clusters               | **At least 2, exact total unknown.** The privacy-stopped confirmation and inconsistent Firefox/French SGN-005 execution are known untriaged outcomes. Six earlier uncertain investigation executions may contain one or more additional clusters, but the sanitized evidence cannot support a distinct-cluster count.                                                                                                                          |
| Published-suite flake candidates | **1/54 execution contexts (1.9%) candidate rate.** On source revision `07343b3`, Firefox/French SGN-005 empty-form validation timed out during the first report attempt while script-load warnings were present, then passed in the immediate complete rerun. The latest committed summary contains only the successful run, so the candidate remains untriaged rather than being labelled an automation flake. Account attempts are excluded. |
...
The proposed coverage denominator groups the approved first-pass goals from the Stage 1 brief into the ten reviewable areas below. The goals are approved, but this exact grouping and denominator remain pending the Stage 8 human decision. Mailbox receipt and active security probes were explicitly deferred, so they remain visible exclusions rather than silently lowering or inflating this proposed denominator.
...
| Accessibility report and severity        | SGN-009 and accessibility baseline               | Yes                              | Zero-result baseline and serious/critical no-regression gate are approved; no manual audit is currently added.   |
| Readability                              | SGN-010                                          | No                               | English and French methods are selected; implementation, observed values, and numeric thresholds remain pending. |
...
- **Proposed inventory mapping:** 10/10 grouped goal areas have a named case or review artifact.
- **Provisional executed-evidence coverage:** 8/10 grouped goal areas have evidence in this snapshot (**80% pending human approval of the denominator**).
