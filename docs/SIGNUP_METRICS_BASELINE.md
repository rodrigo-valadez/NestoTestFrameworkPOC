...
| Published-suite flake candidates | **Not measurable from one comparable run.** The committed Playwright summary labels zero results flaky, but no case has repeated equivalent observations from which to calculate the roadmap's flake-candidate rate. Account attempts used changing diagnostic conditions and are excluded. |
